...
\begin{abstract}
In the process of realising a new product choices need to be made about its preferred configuration. For digitally-controlled products an additional choice needs to be made - its programmed behaviour. 

In this paper, we caricature three extreme practices, contrasting cases where preferences are either \emph{discovered}, \emph{decided} or \emph{designed}. On this analysis, established practice for selecting programmed behaviour does not satisfy the criteria for being \emph{designed}. We examine what obstacles exist to designing product behaviour, and propose a research programme to identify suitable artefacts and processes which can facilitate this activity.
\end{abstract}
...
Design, by Feng and Feenberg's [] broad, inclusive definition is "a process of consciously shaping an artifact to adapt it to specific goals and environments". 
However, not all shaping practices adopt the same world-view. Within some practices, modelled on the scientific method, shaping choices are \emph{discovered}, by establishing facts, from which choices can be deduced. For example, materials' objective physical properties can eliminate them from consideration, or experimental results in human perception recommend employing a particular visual cue for users. Within other practices there is a focus on effectively mapping a space from which the preferred solution will be \emph{decided} by judging amongst them based on an implicit or explicit scheme of values and preference.

In this paper, we reserve the phrase \emph{designed} for shaping choices arising from reflective processes [Fallman][Brock and Craft]. These are neither \emph{discovered} through evidence gathering, nor \emph{decided} through systematic analysis of a space of configurations. Central to a reflective design process are the activities of sketching [] and bricolage [], through which the designer engages in a "conversation with materials" [Schon]. "As designers conceive of a new idea or refine the details of an existing idea, the materials they use begin to “talk back”" [Ozenc et al].

...
\begin{abstract}
In the process of realising a new product choices need to be made about its preferred configuration. For digitally-controlled products an additional choice needs to be made - its programmed behaviour. In this paper, we caricature three extreme practices which are widely used to determine preference, which we label \emph{decided}, \emph{discovered} and \emph{designed}.

On this analysis, it is suggested that the established practice for making choices about programmed behaviour does not satisfy the criteria for being \emph{designed}. We examine what obstacles exist to the design of product behaviour, and propose a research programme to identify suitable artefacts and processes which can facilitate this activity.
\end{abstract}
...
In the broadest terms, design can be thought of as "a process of consciously shaping an artifact to adapt it to specific goals and environments" [Feng and Feenberg]. However, not all shaping practices adopt the same world-view. Some practices conform closely to the scientific method, with a focus on establishing facts, from which choices can be deduced. For example, materials' objective physical properties can eliminate them from consideration, or experimental results in human perception can indicate the superiority of a certain choice of visual representation. Other practices are focused on mapping a space of alternative solutions, and selecting among them according to trade-offs.. Ohher  may be focused on gathering evidence and analysis 
